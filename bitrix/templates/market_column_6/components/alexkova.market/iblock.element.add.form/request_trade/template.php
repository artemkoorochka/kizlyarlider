...
// Asset::getInstance()->addJs("/local/js/jquery-3.3.1.min.js");
$this->setFrameMode(false);
?>
<script src="/local/js/jquery-3.3.1.min.js"></script>
<?
global $BXR_FORM_COUNTER;
if (!empty($arResult["ERRORS"])):?>
	<?ShowError(str_replace('это поле обязательно для заполнения', 'Поле \'Согласие\' обязательно для заполнения!', implode("<br />", $arResult["ERRORS"])))?>
<?endif;
if (strlen($arResult["MESSAGE"]) > 0): $_SESSION['BUY_IN_ONE_CLICK'] = true; ?>
    ...
    return false;?>
<?endif?>
<form id="iblockForm<?=$arParams["IBLOCK_ID"]?>" name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" onsubmit="ym(28058889, 'reachGoal', 'BuyOneClick');">
        <br>
	...
	<input onclick="ym(28058889, 'reachGoal', 'BuyOneClick'); BXReady.Market.formRefresh(<?=$arParams["IBLOCK_ID"]?>);return false;" <?if($arParams["HIDE_SUBMIT"] == 'Y') echo 'style="display: none;"'?> class="color-button bxr-color bxr-color-button bxr-bg-hover-light" id="submitForm_<?=$arParams["IBLOCK_ID"]?>" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
